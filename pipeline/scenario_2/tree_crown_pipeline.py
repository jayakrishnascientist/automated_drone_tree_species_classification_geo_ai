...
class Config:
    ...
    # STEP 1 - Data paths
    ORTHO_FOLDER = '/path/to/orthomosaics'  # Folder with .tif orthomosaics
    POLY_FOLDER = '/path/to/geojson_polygons'  # Folder with crown polygon GeoJSONs
    STEP1_OUTPUT = '/path/to/step1_output'  # Step 1 outputs
    ...
    CHOSEN_K = 2  # Chosen k value (after Step 1 inspection)
    STEP2_OUTPUT = '/path/to/step2_output'  # Step 2 outputs
    
    # STEP 3 - Validation (optional)
    GROUND_TRUTH_CSV = '/path/to/ground_truth.csv'  # Your ground truth
    STEP3_VALIDATION_OUTPUT = None  # Auto-set if not specified
    
    # STEP 4 - KMZ export
    STEP4_OUTPUT = '/path/to/step4_output'  # KMZ output folder
...
def step3_validate(config):
    ...
    if not os.path.exists(config.GROUND_TRUTH_CSV):
        print('⚠️  Ground truth CSV not found. Skipping validation.')
        return
    ...
    # Load ground truth
    gt_raw = pd.read_csv(config.GROUND_TRUTH_CSV)
    print(f'  Ground truth loaded: {len(gt_raw)} rows')
    
    # Auto-detect columns
    gt_img_col, gt_label_col = auto_detect_csv_columns(gt_raw)
    print(f'  Detected image col: {gt_img_col}')
    print(f'  Detected label col: {gt_label_col}')
    ...
    gt_df['true_species'] = normalize_label(gt_df['true_species'])
    
    # Load master
    master_path = os.path.join(config.STEP2_OUTPUT, 'crown_master.csv')
    master_df = pd.read_csv(master_path)
    master_df['pred_species'] = normalize_label(master_df['species'])
    ...
    print(f'\n  Matched samples: {len(val_df)}')
    
    if len(val_df) == 0:
        print('⚠️  No matches found. Check filename format.')
        return
    ...
    metrics_df.to_csv(os.path.join(val_output, 'validation_metrics.csv'), index=False)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Tree Crown Species Classification Pipeline')
